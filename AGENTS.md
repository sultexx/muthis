...
| `src/muthis/cloud/tool_schemas.py` | ~108 | The LOOK-only tool schemas (moved out of claude_agent.py, which sits at the 300-line ceiling; re-exported there so old imports keep working): `highlight_target`, **`draw_shapes` (B-1)** — a LIST of shapes, each `kind` (line/arrow/circle/rectangle) + a uniform (x1,y1,x2,y2) in SENT-image space (arrow head at x2,y2; a circle's 4-tuple is its ENCLOSING bbox — mirrors `shapes.Shape` exactly) + optional `label_ar`; ONE call carrying SEVERAL shapes is the expected composite draw — and `request_screen_refresh`. |
...
| `src/muthis/overlay/shapes_widget.py` | ~120 | Pure VIEW on the Tk thread (Phase A): draws a LIST of ALREADY-PHYSICAL geometric shapes — per-kind NEON line / arrow-headed line / oval (from the circle's bbox) / rectangle, each in TWO glow passes (dim halo + bright core, the arrowhead scaled with the halo) with an optional Arabic caption on a rounded semi-dark chip — all colors/glow/font from the injected `OverlayStyle`, on the SAME shared canvas, ALL items scoped by `SHAPES_TAG` (never delete-all) so the highlight rectangle/pointer are never disturbed; unknown kinds skipped quietly (the Tk thread never dies on bad data). No tkinter import (duck-typed canvas → display-free tests). KNOWN interplay (accepted): a NEW highlight_target wipes drawn shapes because RectangleWidget clears with delete("all") — highlight_target stays untouched. |
...
- **Geometric draw_shapes is a SEPARATE LOOK path — never merged with highlight_target** (`shapes.py` +
  `overlay/shapes_widget.py` + `SidekickOverlay.draw_shapes`, Phase A): line/arrow/circle/rectangle are DRAWN
  ...
  highlight_target, explain-with-illustration → draw_shapes). BOTH tools share the SAME two-pass discipline
