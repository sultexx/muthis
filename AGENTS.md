...
| `src/muthis/overlay/style.py` | ~300 | The SINGLE overlay styling config (Batch 1 neon look + batch 2-A status colors): `OverlayStyle` (frozen; dataclass defaults ARE the neon defaults) + `from_env()` (per-kind colors `MUTHIS_COLOR_*`, `MUTHIS_GLOW`/`_WIDTH`/`_INTENSITY`, `MUTHIS_CORE_WIDTH`, `MUTHIS_LABEL_FONT`/`_SIZE`/`_PLATE`, AND per-state status colors `MUTHIS_STATUS_LISTENING`/`_THINKING`/`_SPEAKING` + the dot corner `MUTHIS_STATUS_CORNER` via `_env_corner` — malformed values warn + fall back). `status_colors` (cyan/amber/green; idle is ABSENT = hidden) + `status_corner` (default bottom-right) are dataclass fields so `OverlayStyle()` stays deterministic. Helpers: `glow_strokes()` (outer dim halo + inner bright core — glow EMULATED, Tk has no alpha), `dim()` (halo tint toward black), `color_for()`, `caption_font()`, `round_rect_points()` + `draw_caption_chip()` (rounded semi-dark chip). `_safe_color()`/`_avoid_key()` keep any color off the #FF00FF transparent key. NO tkinter import (duck-typed canvas) so the headless widgets/tests import it freely. Styling ONLY — never coordinates/ghosting/sync/threading. |
